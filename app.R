                ...
                fluidPage(
                  ...
                  wellPanel(style = "background: white",
                      ...
                      ")
                            ...
                            hr(),
                            h4(strong("Follow the instructions in each of the numbered items within this tab"))
                  ),
                  hr(),
                  fluidRow(
                    column(4,
                          wellPanel(style = "background: white",
                           ...
                           wellPanel(style = "background: white",
                                     ...
                                     hr(),
                                     h5("First you must Run WEAP and Extract precipitation and streamflow (section 1)"),
                                     hr(),
...
")),
  ...
  server = function(input, output) { 
    ...
    VAL = reactiveValues(Model=0, Model1=0, Model2=0,Model3=0)
        ...
        if (dir.exists(modelfolder) && Warea!="") {
          ...
          if (file.exists(paste0("ResultsGauges.csv")) && Model==1) {
            ...
            output$StreamflowA <- renderUI({
              ...
              gauges=sort(unique(ResultsGauges$Gauge))
              VAL$gauges1=gauges
      ...
      if (Model==1){
        ...
        withProgress(message = 'Calculation in progress',
                     detail = 'This may take a while...', value = 0, {
                       ...
                       var="Key\\Num:Annual Activity Level" 
                       res <- try(WEAP$BranchVariable(var)[["Expression"]] <- 1)
                       ...
                       files=c(files,"Results_Q_P_monthlyTimeserie.csv","Results_Q_P_monthlySummary.csv","ResultsGauges.csv",paste0(RUNID,"_",Scen,"_WaterBalance.csv"),paste0(RUNID,"_",Scen,"_ResultsGauges.csv"))
                       ...
                       cols=c("Year", "Time step", "Gauge","Observed [M^3]")
                       #str(resultsWBG)
        ...
        if (file.exists(paste0("ResultsGauges.csv")) && Model==1) {
          ...
          #ResultsGauges=ResultsGauges[which(ResultsGauges$Gauge==unique(ResultsGauges$Gauge)[1]),]
          ...
          ResultsGauges$Dates=ymd(myDates)
          ...
          for (i in 1:length(unique(obs$Gauge))){
            ...
            flows$ks <- interflowd/(as.numeric(z1)/100)^2 
            
            bfdepth <- low/flows$Area*1000 
            flows$kd <- bfdepth/(as.numeric(z2)/100)^2 
          ...
          write.csv(obs[,c(-4,-5)],paste0("Resultsk","-DSR",srpercent,"-Z1",z1,"-Z2",z2,".csv"),row.names=F) 
          ...
          output$textRunEnsembleAConduc1 <- renderText({ 
            ...
            outTxt = ""
            text=paste0("Conductivity was calculated for "," DSR: ",srpercent," Z1: ",z1," Z2: ",z2) 
            formatedFont = sprintf('<font color="%s">%s</font>',"green",text)
            outTxt = paste0(outTxt, formatedFont)
          ...
          output$kestimate <- DT::renderDataTable({
            #table <- read.csv(paste0("Resultsk_Summary-DSR30-Z130-Z250.csv"), stringsAsFactors=F, check.names=F)
            table <- read.csv(paste0("Resultsk_Summary","-DSR",srpercent,"-Z1",z1,"-Z2",z2,".csv"), stringsAsFactors=F, check.names=F)
          ...
          if (file.exists(paste0("Resultsk","-DSR",srpercent,"-Z1",z1,"-Z2",z2,".csv")) && Model==1){
            ...
            file <- read.csv(paste0("Resultsk","-DSR",srpercent,"-Z1",z1,"-Z2",z2,".csv"), stringsAsFactors=F, check.names=F)
            #file <- read.csv(paste0("Resultsk-DSR20-Z130-Z230.csv"), stringsAsFactors=F, check.names=F)
      ...
      if (Model==1){
        withProgress(message = 'Calculation in progress',
                     detail = 'This may take a while...', value = 0, {
                       ...
                       var="Key\\Num:Annual Activity Level" 
                       res <- try(WEAP$BranchVariable(var)[["Expression"]] <- 2)
    ...
    observe({
      ...
      if (!is.null(name)){
        ...
        shinyjs::show("Q")
        shinyjs::show("Qmonthly")
        shinyjs::show("fdc")
        shinyjs::show("WB")
        shinyjs::show("WBSM")
        shinyjs::show("WBSE")
        shinyjs::show("WBmonthly")
        ...
        output$WBSE <- renderPlotly({
          ...
          p5 <- plot_ly(SMmonthly, x=~Month, y=SMmonthly[,"SM"], name = "Soil Moisture", showlegend=FALSE,type="bar",
                        line = list(color="#CF6D0C",width=1.5),text=~paste("Decrease Surface Storage = ", SMmonthly$SM)) %>%
        ...
        shinyjs::hide("metricsp")
        shinyjs::hide("Q")
        shinyjs::hide("Qmonthly")
        shinyjs::hide("fdc")
        shinyjs::hide("WB")
        shinyjs::hide("WBSM")
        shinyjs::hide("WBSE")
        shinyjs::hide("WBmonthly")
        shinyjs::hide("WBmonthlyB")
    ...
    observe({
      ...
      if (file.exists(paste0("KeyModelInputs.csv")) &&  Model==1){
        ...
        if (file.exists(name)){
          ...
          #gauge=unique(file$Gauge)[1]
          #file=file[file$Gauge==gauge,]
          file=file[file$Gauge==input$StreamflowSelect1,]
          file$YearMonth=year(file$Dates)*100+month(file$Dates)
          file$Month=month(file$Dates)
      ...
      if (!is.null(name)){
        ...
        shinyjs::show("metrics1")
        shinyjs::show("Q1")
        shinyjs::show("Qmonthly1")
        shinyjs::show("fdc1")
        shinyjs::show("WB1")
        shinyjs::show("WBSM1")
        shinyjs::show("WBSE1")
    ...
    try({
        ...
        shinyjs::hide("metricsp1")
        shinyjs::hide("Q1")
        shinyjs::hide("Qmonthly1")
        shinyjs::hide("fdc1")
        shinyjs::hide("WB1")
        shinyjs::hide("WBSM1")
        shinyjs::hide("WBSE1")
        shinyjs::hide("WBmonthly1")
        shinyjs::hide("WBmonthlyB1")
        shinyjs::hide("WBtable1")
      }
